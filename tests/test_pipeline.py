...
from pipeline.pipeline import run_rag_pipeline
...
def test_pipeline_naive_returns_dict():
    result = run_rag_pipeline(str(FIXTURE), "What is an architectural pattern?", mode="naive")
...
def test_pipeline_naive_sources_are_strings():
    result = run_rag_pipeline(str(FIXTURE), "what is a component?", mode="naive")
